...
output: 
  pdf_document:
    ...
    toc: true
    toc_depth: 4
title: "Computer Project 1: SEMMA w/ Regularized Logistic Regression"
author: 
...
head(data)[c(1,2,3),]
```
I was able to load in the dataset using the builtin R function 'read.csv()'. Afterwards I can inspect the dimensions, summarize the dataset, and view the first three rows/samples.
...
- Are there missing values? If so, handle them with an appropriate strategy such as listwise
...
- For a continuous predictor, use the parametric two-sample $t$ test or the nonparametric
Wilcoxon rank-sum test.

- For a categorical predictor, use the $\chi^{2}$ test of independence or Fisher's exact test in case
...
a) Fit the regularized logistic regression using the training data D1. While 'l1' regularization
or LASSO is suggested here, you may use other penalty functions of your choice.
